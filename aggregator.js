...
const aggregate = () => {
        ...
        db.ref('ONTARIO-OCS/capsules').once('value').then(function(snapshot) {
            ...
            capsuleArray.forEach((product) => {
                for (let i = 0; i < product.options[0].values.length; i++) {
                    ...
                    // model.pricePerCapsule   =  product.variants[i].price.toInt() / matches
    ...
    let scrapeVapes = () => {
        db.ref('ONTARIO-OCS/cartridges').once('value').then((snapshot) => {
            let allVapes = snapshot.val();
            let arr = [];
            let vapeArr = [];
            let model = {};
            let vapeStatsArr = [];
            ...
            // load all the pre roll stuff into an array
            for (let i = 0; i < Object.keys(allVapes).length; i++) { // number of dates in the collection
                let date = Object.keys(allVapes)[i];
                arr.push(allVapes[date])
            }
            // flatten out the array
            arr = arr.flatMap(({products}) => products);
            vapeArr = _.uniqBy(arr, 'id');
            vapeArr.forEach((product) => {
                for (let i = 0; i < product.options[0].values.length; i++) {
                    ...
                    model.cbd               =  cbd[0].replace("cbd_content_max--", "");
                    model.id                =  product.id;
                    model.gramsEquiv        =  product.options[0].values[0];
                    model.brandName         =  product.vendor;
                    model.sku               =  product.variants[i].sku;
                    model.price             =  product.variants[i].price;
                    model.is510             =  product.body_html.includes('510') ? 'true' : 'n/a';
                    model.is510             =  product.handle.includes('510') ? 'true' : 'n/a';
                    model.isC02             =  product.body_html.includes('C02') ? 'true' : 'n/a';
                    model.productName       =  product.handle;
                    vapeStatsArr.push(model);

                    model = {};
                }
            });
            let pageRefPreRoll = metaStatsOCS.child( 'vapeStatisticsAsArrayOfProducts/' + dateToString);
            pageRefPreRoll.set(vapeStatsArr);
            console.log('set vape ============')
